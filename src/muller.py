...
def _muller_plot(populations_df, adjacency_df, color_by, colormap='terrain', colorbar=True, background_strain=True,
    ...
    if colorbar:
        ...
        cb.set_label(color_by.name,rotation=270,labelpad=15)
        cb.ax.yaxis.label.set_fontproperties(matplotlib.font_manager.FontProperties(family='Arial', size=12))
    
    ax.set_xlabel('Time', fontdict={'family': 'Arial', 'size': 12})
    ax.set_ylabel('Frequency' if normalize else 'Abundance', fontdict={'family': 'Arial', 'size': 12})
